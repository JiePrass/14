...
const getUserProfile = async (req, res) => {
  try {
    // Ambil seluruh user dan urutkan berdasarkan point
    const users = await User.findAll({
      attributes: ["id", "point"],
      order: [["point", "DESC"]],
    });
    ...
    // Hitung ranking user berdasarkan point
...
const updateUser = async (req, res) => {
  try {
    ...
    // Jika ada perubahan password, hash ulang
    if (updateData.password) {
      updateData.password = await bcrypt.hash(updateData.password, 10);
    }

    // Pastikan profile_picture ada sebelum mengaksesnya
